    ...
    while (pilha):
        ...
        if v == fim:
            ...
            for i in range(len(caminho)-1):
                flux_minimo =  G[caminho[i]][caminho[i+1]][1] - G[caminho[i]][caminho[i+1]][0]
...
def main():
        ...
        for i in range(nTimes):
            for j in range(i + 1, nTimes):
                ...
                if jogos_restantes and jogos_restantes > 0:
                    node = f"jogo_{i}_{j}"
                    T = add_aresta(T,fonte,node,0,jogos_restantes)

                    T = add_aresta(T, node, f'time_{i}', 0,float('inf'))
                    T = add_aresta(T, node, f'time_{j}', 0,float('inf'))
    ...
    # for u in T:
    #     for v in T[u]:
    #         print(f"{u} -> {v}, fluxo/capacidade = {T[u][v]}")
